...
img = cv2.inRange(img, h_min, h_max)
...
for line in lines:
    ...
    for x1,y1,x2,y2 in line:
     if math.hypot(x2-x1, y2-y1)>10:
      ...
      print (pAngle)
      cv2.putText(source, str(round(pAngle,1)), (int (x1) , int (y1)), cv2.FONT_ITALIC, 0.4, 255) # # cv2.putText(frame,text,location,font,font size,font color, font weight, line)
      cv2.putText(source, str(number), (int (x1)+45, int (y1)+45), cv2.FONT_ITALIC, 4, 255)
      number +=1
plt.imshow(source)
plt.show()
